...
func TestBasic(t *testing.T) {//(1) 100 Tests Passed
  ...
  var gid1 int64 = 1
  ck.Join(gid1, []string{"x", "y", "z"})//First group of servers
  check(t, []int64{gid1}, ck)
  cfa[1] = ck.Query(-1)

  var gid2 int64 = 2
  ck.Join(gid2, []string{"a", "b", "c"})//Second group of servers
...
func TestUnreliable(t *testing.T) {//(2)
  runtime.GOMAXPROCS(4)
...
func TestFreshQuery(t *testing.T) {//(3)
  runtime.GOMAXPROCS(4)
