...
func _TestBasic(t *testing.T) {//(1) PASSED 50
  ...
  v := ck.PutHash("a", "b")
  if v != "x" {
    t.Fatalf("Puthash got (%s) wrong value",v)//TODO remove %s
  }
  ov := NextValue("x", "b")
	mv:=ck.Get("a")
  if mv != ov {
    t.Fatalf("Get got (%s) wrong value (%s)",mv,ov)//TODO remove %s
  }
...
func _TestMove(t *testing.T) {//(2) PASSED 50
...
func _TestLimp(t *testing.T) {//(3) PASS 50
  ...
  for g := 1; g < len(gids); g++ {
    ...
    for i := 0; i < len(keys); i++ {
	//fmt.Println("Get  " + keys[i])//TODO Erase
      v := ck.Get(keys[i])
      if v != vals[i] {
        t.Fatalf("joining; wrong value; g=%v k=%v wanted=%v got=%v",
          g, keys[i], vals[i], v)
...
func _TestConcurrent(t *testing.T) {//(4) PASSED 150 TESTS
  fmt.Printf("Test: Concurrent Put/Get/Move ...\n")
  doConcurrent(t, false)
  fmt.Printf("  ... Passed\n")
}

func TestConcurrentUnreliable(t *testing.T) {//(5) PASSED 100 TESTS
