...
func TestAtMostOnce(t *testing.T) {
  ...
  for i := 0; i < 100; i++ {
    v := strconv.Itoa(i)
    pv := ck.PutHash(k, v)
	//fmt.Println("Client Concat: " + (val + v) + " hashed " + strconv.Itoa(int(hash(val + v))))
...
func TestRepeatedCrashUnreliable(t *testing.T) {
  ...
  go func() {
    ...
    for done == false {
      ...
      // fmt.Printf("%v killing %v\n", ts(), 5001+i)
      sa[i].kill()
	//fmt.Println("Died & Restarted: " + sa[i].me)
      // wait long enough for new view to form, backup to be initialized
      time.Sleep(2 * viewservice.PingInterval * viewservice.DeadPings)
  ...
  for xi := 0; xi < nth; xi++ {
    cha[xi] = make(chan bool)
    go func(i int) {
	//fmt.Println("Thread" + strconv.Itoa(i))
	//time.Sleep(1000 * time.Millisecond)
      ...
      for done == false {
        v := strconv.Itoa(n)
        pv := ck.PutHash(k, v)
	//fmt.Println("Client"+k+" Concat: " + (data[k]+ v) + " hashed " + strconv.Itoa(int(hash(data[k]+ v))))
	//time.Sleep(1000 * time.Millisecond)
        if pv != data[k] {
        ...
        v = ck.Get(k)
        if v != data[k] {
		//fmt.Println("Contacting: " + ck.vs.Primary())
