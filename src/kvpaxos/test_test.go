...
func _TestBasic(t *testing.T) {//(1) 100 PASSED
...
func _TestDone(t *testing.T) {//(2) 100 PASSED
  runtime.GOMAXPROCS(4)
...
func _TestPartition(t *testing.T) {//(3) 100 PASSED
  runtime.GOMAXPROCS(4)
...
func _TestUnreliable(t *testing.T) {//(4) (?)
  runtime.GOMAXPROCS(4)
  ...
  for i := 0; i < nservers; i++ {
    kva[i] = StartServer(kvh, i)
    kva[i].unreliable = true//TODO true
  }
  ...
  const ncli = 5
	...
	count:=0//TODO
      go func(me int) {
	//fmt.Println("Count: ("+strconv.Itoa(me)+","+strconv.Itoa(count)+")")//TODO
        ok := false
        defer func() { ca[me] <- ok }()
        sa := make([]string, len(kvh))
        copy(sa, kvh)
        for i := range sa {//Talk to servers in different order
          j := rand.Intn(i+1)
          sa[i], sa[j] = sa[j], sa[i]
        }
        myck := MakeClerk(sa)
        key := strconv.Itoa(me)//Client handles unique key

        pv := myck.Get(key)// Get key --> ""
        ov := myck.PutHash(key, "0")// PutHash --> ""
	//fmt.Println("Client[ "+key+" , " + ov +"]("+strconv.Itoa(iters)+")")//AA
        if ov != pv {
          t.Fatalf("wrong value(%s); expected %s but got %s", key,pv, ov)//TODO
        }
	//fmt.Println("Client[ "+key+" , " + pv +"]("+strconv.Itoa(iters)+")")//XX
        ov = myck.PutHash(key, "1")
        pv = NextValue(pv, "0")
        if ov != pv {
          t.Fatalf("wrong value(%s); expected %s but got %s", key,pv, ov)
        }
	//fmt.Println("Client[ "+key+" , " + pv +"]("+strconv.Itoa(iters)+")")//BB
        ...
        nv := NextValue(pv, "2")
	//fmt.Println("Client[ "+key+" , " + nv +"]("+strconv.Itoa(iters)+")")//CC
//        time.Sleep(3000 * time.Millisecond)//TODO 100 millisecond
	time.Sleep(100*time.Millisecond)
	ov = myck.Get(key)
        if myck.Get(key) != nv {
	//if ov!=nv{
          //t.Fatalf("wrong value")//TODO: Change to
		t.Fatalf("wrong value(%s); expected %s but got %s", key,nv,ov)
        }
	fmt.Println("Client[ "+key+" , " + nv +"]("+strconv.Itoa(iters)+")")//DD
	//ov = myck.Get(key)
        if myck.Get(key) != nv {
	//if ov!=nv{
          t.Fatalf("wrong value(%s); expected %s but got %s", key,nv,ov)
...
func _TestHole(t *testing.T) {//(5) PASSED 100
  runtime.GOMAXPROCS(4)
...
func TestManyPartition(t *testing.T) {//(6) PASSED
  runtime.GOMAXPROCS(4)
